...
public class WaitHelper {
    ...
    public WaitHelper waitForElementNotVisible(WebElement element) {
        try {
            ...
            new WebDriverWait(getDriver(), DEFAULT_TIMEOUT).until(ExpectedConditions.invisibilityOf(element));
            return this;
        } catch (WebDriverException e) {
    ...
    public WaitHelper waitForUrlChangesFrom(String oldUrl){
        log.info("Waiting for url change from " + oldUrl);
        Boolean until = new WebDriverWait(getDriver(), DEFAULT_TIMEOUT)
                .until((ExpectedCondition<Boolean>) driver -> {
            String url = driver != null ? driver.getCurrentUrl() : null; //nuynn a inch vor grenq if-ov
